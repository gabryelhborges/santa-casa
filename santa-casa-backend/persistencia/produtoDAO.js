...
export default class ProdutoDAO{
    async gravar(produto){
        if(produto instanceof Produto){
            const sql = "INSERT INTO produtos(prod_ID, Fabricante_idFabricante ,nome,psicotropico,valor_custo,far_cod,observacao,descricao_uso, tipo) VALUES (?,?,?,?,?,?,?,?,?,);";
    ...
    async atualizar(produto){
        if(produto instanceof Produto){
            const sql = "UPDATE produtos SET  Fabricante_idFabricante  = ?,nome = ?,psicotropico = ?,valor_custo = ?,far_cod = ?, observacao = ?,descricao_uso = ?, tipo = ? WHERE prod_ID = ?";
            const parametros = [ produto.Fabricante_idFabricante,produto.nome,produto.psicotropico,produto.valor_custo,produto.far_cod,produto.observacao,produto.descricao_uso,produto.tipo, produto.prod_ID];
            const conexao = await conectar();
    ...
    async consultar(termo){
        ...
        for(const registro of registros){
            const produto = new Produto(registro.prod_ID, registro.Fabricante_idFabricante, registro.nome, registro.psicotropico, registro.valor_custo, registro.far_cod, registro.observacao, registro.descricao_uso, registro.tipo);
