...
export default class ItensConsumoDAO {
    ...
    async relatorioProdutosConsumidos(itCons, conexao, dataInicio, dataFim) {
        let sql;
        let parametros = [];

        sql = `SELECT p.nome, l.total_conteudo, ic.ic_lote_codigo, u.unidade, sum(ic.ic_qtdeConteudoUtilizado) total 
       FROM ItensConsumo ic 
       INNER JOIN Produtos p ON ic.ic_prod_id = p.prod_ID
       INNER JOIN Lote l ON l.produto_prod_ID = p.prod_ID
       INNER JOIN Unidade u ON u.un_cod = p.un_min
       INNER JOIN Consumo c ON c.cons_id = ic.ic_cons_id`;

        if (itCons.produto && dataInicio && dataFim) {
            sql += ` WHERE p.nome LIKE ? AND DATE(c.cons_dataConsumo) >= ? AND DATE(c.cons_dataConsumo) <= ?`;
            parametros = ['%' + itCons.produto.nome + '%', dataInicio, dataFim];
        }
        else {
            if (dataInicio && dataFim) {
                sql += ` WHERE DATE(c.cons_dataConsumo) >= ? AND DATE(c.cons_dataConsumo) <= ?`;
                parametros = [dataInicio, dataFim];
            }
            else {
                if (dataInicio || dataFim) {
                    if (dataInicio) {
                        sql += ` WHERE DATE(c.cons_dataConsumo) >= ?`;
        ...
        sql += ` GROUP BY p.nome, l.total_conteudo, ic.ic_lote_codigo, u.unidade
         ORDER BY p.nome;`;
